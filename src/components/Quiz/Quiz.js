...
const Quiz = ({quiz}) => {
    ...
    return (
        <div className='bg-blue-400 hover:bg-blue-900 mb-10 mr-2 ml-2 border rounded-md'>
            ...
            <Link to={`/quiz/${id}`}>
            <button onClick={()=>handleStartQuiz(name)} className='border-y-4 border-Neutral-500  bg-blue-500 hover:bg-blue-700 font-serif hover:font-sans  w-40  text-red-50 font-mono p-1 rounded mt-4 mb-4 hover:bg-lime-900'>Start Quiz</button>
            </Link>
            </div>
        </div>
    );
